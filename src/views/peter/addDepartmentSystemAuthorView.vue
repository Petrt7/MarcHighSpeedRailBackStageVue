<template>
    ...
    <div class="container text-center">
        <div class="row align-items-start">
            <div class="col"></div>
            <div class="col">
                <select class="form-select" v-model="deptName">
                    <option v-for="(dept, index) in json.depts" :key="index">
                        {{ dept.id }}:{{ dept.name }}
                    </option>
                </select>
            </div>
...
async function getAllDepartments() {
    const res = await httpClient.get("/department/all");
    json.depts = res.data;
}
...
async function addAuthor() {
    ...
    httpClient
        .post("/system-author/dept/add", author)
        .then((res) => {
            console.log(res);
